...
def create_inventory(inv_in: schemas.InventoryCreate, db: Session = Depends(get_db), current_admin: models.Admin = Depends(get_current_admin)):
...
def remove_inventory(inventory_id: int, db: Session = Depends(get_db), current_admin: models.Admin = Depends(get_current_admin)):
